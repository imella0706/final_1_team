...
def _load_dag_module_with_fake_airflow(monkeypatch: pytest.MonkeyPatch) -> Any:
    monkeypatch.setenv("BRANDMATE_SNS_TREND_YOUTUBE_LANDING_SCHEDULE", "10 20 * * 3")
...
def test_dag_defaults_to_weekly_schedule(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert module.sns_trend_youtube_landing_collection.dag_kwargs["schedule"] == "10 20 * * 3"
...
def test_resolve_youtube_landing_config_requires_data_interval_end_for_defaults(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    module = _load_dag_module_with_fake_airflow(monkeypatch)
    ...
    with pytest.raises(module.AirflowException, match="data_interval_end is required"):
        module._resolve_youtube_landing_config(
            conf={},
...
def test_resolve_youtube_landing_config_uses_data_interval_end_for_scheduled_runs(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    module = _load_dag_module_with_fake_airflow(monkeypatch)
    ...
    assert config["keyword_csv"].endswith(
        "week=2026-W31/raw/youtube/"
        "run_id=scheduled__2026-07-22T20:10:00+00:00/"
        "youtube_keywords_2026-07-30.csv"
    )
