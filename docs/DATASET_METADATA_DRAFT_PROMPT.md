...
3. 기존 폴더의 파일 역할과 데이터 흐름을 기준으로, 표준 GCS 폴더 구조 초안을 설계해주세요.
   - 각 기존 경로가 GCS의 어느 경로로 이동하는지 경로 매핑 표를 포함해주세요.
   - 확정되지 않은 경로는 추측하지 말고 `TODO`로 표시해주세요.
4. 분석 결과를 기반으로 `manifest.json` 초안을 만들어주세요.
5. 분석 결과를 기반으로 `description.md` 초안을 만들어주세요.
...
출력 형식:
1. 데이터 단계 판단
2. 기존 데이터셋 폴더 분석 기반 GCS 폴더 구조 및 경로 매핑 표
3. manifest.json
4. description.md
   - 공통 필수 섹션을 포함하고, 제공된 상세 정보는 생략하지 않은 완성형 초안으로 작성
5. 담당자가 직접 확인해야 할 TODO 목록
