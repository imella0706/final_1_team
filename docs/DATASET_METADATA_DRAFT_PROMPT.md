...
- `status`는 `manifest.json`에 작성해주세요. 허용값은 `experimental`, `baseline`, `stable`, `deprecated`입니다.
- 현재 운영 범위에서는 중앙 manifest 사본, catalog/index, GCS metadata 사본을 만들지 않습니다.
- `manifest_gcs_path`, `data/manifests/{dataset_name}_{version}.json`, `docs/datasets/catalog/{dataset_name}.json`, artifact 내부 `docs/manifest.json` / `docs/description.md` 경로를 만들지 마세요.
- `description.md`의 Storage 섹션에도 GCS metadata 사본 경로를 쓰지 마세요. GCS에는 데이터 파일만 업로드한다고 작성하세요.
...
- curated 데이터셋 경로는 `data/curated/{dataset_name}/{version}/{artifact_name}/` 구조로 작성해주세요.
...
- 중앙 `data/manifests/` 경로와 `docs/datasets/catalog/` catalog는 현재 사용하지 않습니다.
- GCS에는 `manifest.json`, `description.md` 사본을 두지 않습니다. metadata는 Git의 `docs/datasets/...` 아래에서만 관리합니다.
...
분석할 기존 데이터셋 폴더:
- 'TODO' (예시: final_1_team/data/5gb_v2_diverse)
- 필요하면 하위 파일과 폴더를 직접 읽어 분석하세요.

생성할 문서:
- `docs/datasets/{dataset_name}/{version}/{artifact_name}/manifest.json`
- `docs/datasets/{dataset_name}/{version}/{artifact_name}/description.md`
```
