        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "입력된 매장의 1차 공공 상권 데이터(유동인구, 연령대)와 2차 CCTV 관측 데이터(피크시간, 보행동선)를 정밀 융합하여 "
                    "매장 점주님을 위한 맞춤형 경영·마케팅 컨설팅 리포트를 작성하십시오.\n\n"
                    "🔴 [어조 및 작성 준수 사항 (Mandatory Rules)]:\n"
                    "1. 절대 반말(~하라, ~할 것)을 사용하지 말고, 매장 점주님께 전달하는 정중하고 전문적인 높임말(~하십시오, ~를 제안해 드립니다, ~로 분석됩니다)을 사용하십시오.\n"
                    "2. 뻔한 일반론을 배제하고, 입력 데이터에 명시된 주요 수치(유동인구, 메인 타깃 비율, 피크 관측 인원, 동선 비율 등)를 직접 인용하여 분석 근거로 제시하십시오.\n"
                    "3. 입력된 연령대와 보행 동선 특성에 가장 적합한 메뉴 라인업과 홍보물(입간판/배너) 배치를 맞춤형으로 제안하십시오.\n"
                    "4. 한 줄 요약이 아닌, 각 항목당 4~6줄 분량의 깊이 있는 전략적 분석과 실행 방안을 상세히 설명하십시오.\n\n"
                    "🟢 [4개 핵심 리포트 작성 가이드]:\n"
                    "- [카페 상권 & 타겟 분석]: 입력된 메인 연령대 유동 비중을 짚고, 해당 타깃층이 선호하는 시그니처 음료 및 메뉴군 중심의 타깃 마케팅 방향을 작성하십시오.\n"
                    "- [CCTV 관측 & 픽업 피크 평가]: 실측 피크 시간대 관측 인원과 보행 동선 흐름을 분석하여, 피크 타임 전 테이크아웃 픽업 대기 동선과 사전 제조 준비 절차를 가이드하십시오.\n"
                    "- [카페 맞춤 메뉴 & 액션 플랜]: 타깃층의 특성(예: 대형 폰트 필요성 등)과 보행 유입 동선을 고려한 입간판/배너 제작 및 전방 배치 마케팅 실천안을 제안하십시오.\n"
                    "- [카페 POS 성과 & 회전율 검증]: 도입 후 POS 데이터(테이크아웃 비중 목표, 시간대별 음료 수량, 쿠폰 회수율)를 일주일 단위로 비교 검증하는 구체적 성과 추적 체계를 제시하십시오.\n\n"
...
def generate_l4_artifacts():
    ...
    # Dynamic Data Context for Store
    store_context = {
        "store_name": "탐앤탐스 (동대문구 제기동점)",
        "location_name": "서울특별시 동대문구 제기동 (청량리역·서울약령시장 주변 상권)",
        "daily_traffic": 48500,
        "age_1020": 12.4,
        "age_3040": 31.8,
        "age_5060": 55.8,
        "main_target_desc": "5060 어르신 세대 (55.8% - 서울시 최상위 비중)",
        "peak_window": "12:00 ~ 13:00",
        "peak_count": 3.78,
        "roi_share": 48.1,
        "flow_down": 50.0,
        "flow_up": 50.0,
        "poi_info": "청량리역 (250m), 서울약령시장 입구 버스정류장 (80m)"
    }

    print(f"[INFO] Found OpenAI API key in {env_path}. Triggering DYNAMIC OpenAI LLM Synthesis Call for {store_context['store_name']}...")
    prompt = f"""
    [1차 정량 데이터 상세 입력 - {store_context['store_name']}]
    - 매장 입지: {store_context['location_name']}
    - 상권 유동인구: 일평균 {store_context['daily_traffic']:,}명 (상권 내 보행 흡수율 상위 18%)
    - 연령대 분포: 10-20대({store_context['age_1020']}%), 30-40대({store_context['age_3040']}%), 메인 타깃: {store_context['main_target_desc']}
    - CCTV 관측 데이터 (매장 전면): 
      * 피크 시간대: {store_context['peak_window']} (프레임당 평균 {store_context['peak_count']}명 관측, 상대 혼잡도 피크)
      * 매장 앞 관측구역 비중: 전체 유동 관측의 {store_context['roi_share']}%
      * 보행 이동 방향: 화면 하향 {store_context['flow_down']}% vs 화면 상향 {store_context['flow_up']}% 양방향 동일 비중
    - 인근 집객 시설: {store_context['poi_info']}
    
    위 정량 데이터를 종합 분석하여 입지 점수(overall_score), 백분위, 예상 상승률, 4단 AI 컨설팅 리포트를 산출해 주십시오.
    """
