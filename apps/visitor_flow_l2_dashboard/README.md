...
- 조사 대상, 위치, 기간, 실제 관측 시간대와 분석 범위
- 분석 범위 내 가장 높은 관측 시간대와 분석 장면당 평균 인원
- 날짜별 시간대 관측 추이
- 매장 앞 관측 비중과 개인정보 보호 처리된 대표 이미지
- 두 날짜에 모두 영상이 있는 공통 시간대 비교
- 근거 수치에 연결된 운영·마케팅 실험 제안
- 방문객 총수, 하루 총량, 매출 효과로 확대 해석하지 않기 위한 활용 기준

고객 PDF 본문에서는 `ROI`, `bbox`, `sampled frame`, `threshold`, `confidence`, `AP/F1`, `6x4 grid` 같은 구현·검증 용어를 노출하지 않습니다. 각각 `매장 앞 관측구역`, `분석 장면`, `분석 장면당 평균 인원`처럼 고객이 바로 이해할 수 있는 표현으로 바꿉니다.

고객 PDF에서 `관측량`은 CCTV 화면에서 사람으로 탐지된 횟수이며, 방문자 수나 통행량을 의미하지 않습니다.

고객용 용어 기준:

| 내부/분석 용어 | 고객용 표현 | 설명 |
| --- | --- | --- |
| ROI | 매장 앞 관측구역 | 노란 선 안쪽처럼 분석할 구역 |
| 매장 전면 관심구역 | 매장 앞 관측구역 | 고객 리포트 제목과 본문에서 사용 |
| 매장 전면과 계단 진입 동선 | 매장 앞 보행로와 계단 진입로 | 실제 장소 설명에 사용 |
| 보행 관측량 | 관측량 | CCTV 화면에서 사람으로 탐지된 횟수 |
| 영역 카운팅 | 정해진 구역 안에 보인 사람을 세는 방식 | 본문에는 필요할 때만 짧게 설명 |
| 장면당 평균 관측 | 분석 장면당 평균 인원 | 일정 간격으로 확인한 화면에서 동시에 보인 사람 수의 평균 |
