...
모델의 목적은 음식 종류를 분류하거나 전경 마스크를 만드는 것이 아니다. 원본 사진과 생성 배경의 원근을 맞춰 합성 결과가 공중에 떠 보이는 문제를 줄이는 것이다. 현재 preserve 모드의 전경 배치 기본 목표는 탑뷰 `0.40`, 45도 `0.48`이며 허용 범위는 0.30~0.70이다. 후보 배경 생성·선택과 OpenCLIP·배경 음식·기하 검증의 상세 동작은 `CAMERA_ANGLE_PROMPT_SELECTION.md`를 참고한다.
