...
python -m scripts.download_models --models yolo sam2 big-lama openclip birefnet sana
...
실행 보고서 `data/reports/<입력명>_background_replacement_report.json`의 `debug_artifacts`를 확인한다. 원본 SAM과 안정화 SAM 마스크를 먼저 비교한다. 안정화 마스크는 내부 투명 구멍과 작은 잡음을 제거한 합성용 마스크다. 이어서 BiRefNet/SAM 알파, RGBA 전경 미리보기, `semantic_*_reference`·`semantic_*_candidate` 전경 비교 이미지를 확인한다. OpenCLIP 검증에 실패한 경우에는 최종 합성 대신 거부된 합성 이미지가 기록된다.
