...
이 모델의 역할은 음식 종류 분류가 아니라, GroundingDINO 후보와 함께 배경 교체 파이프라인의 SAM 2.1 Small에 전달할 음식 사각형을 찾는 것이다.
...
- 배경 제거 정확도는 탐지 상자와 SAM 2.1 Small 마스크 품질에 좌우된다. 현재 기본 경로에서는 BiRefNet을 사용하지 않는다.
...
이 검증은 음식 분할을 대신하지 않는다. 입력 전경은 GroundingDINO·YOLO 상자를 SAM 2.1 Small에 전달해 분할하고, 배경 후보 검증은 음식 없는 광고 배경이라는 조건을 확인하는 별도 안전장치다.
