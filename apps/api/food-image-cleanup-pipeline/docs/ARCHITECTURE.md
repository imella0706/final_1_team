...
이 문서는 `BackgroundReplacementPipeline.run()`과 `configs/pipeline.yaml`의 현재 상태를 기준으로 한다. 이번 대조 기준은 2026년 7월 29일의 코드 커밋 `11a710f`다. 파이프라인은 원본 음식과 접시를 가능한 한 보존하고, 불필요한 주변 물체를 제거한 뒤 생성한 빈 광고 배경과 합성한다.
...
→ PlateMaskService 용기 형태 판정과 접시 전체 마스크 정리
→ preserve 모드의 음식 연결 꼬치·지지 구조 안전 복구
→ 음식 마스크·접시 마스크 독립 품질 검사
...
| 음식 전용 YOLO11n | `food_specialized` | 추론은 실행하지만 GroundingDINO가 놓친 음식 또는 용기 쪽만 최종 상자로 보완한다. |
...
| PlateMaskService | `enabled: true` | 용기를 `ellipse`, `quadrilateral`, `irregular`로 분류한다. 저신뢰 형태는 임의 완성 대신 원본 연결 영역을 유지한다. |
| 음식·접시 마스크 품질 | 분리 검사 | 음식의 파편화와 접시의 구멍·윤곽 안정성을 서로 다른 기준으로 검사한다. |
| 음식 지지 구조 복구 | `enabled: true`, preserve 전용 | GroundingDINO 후보, Canny/Hough 선분, 음식 연결 증거를 모두 통과한 꼬치형 구조만 보호한다. |
...
| 접시 림 복원 | `enabled: true`, 적응형 관찰 | `preserve_original_plate`에서 실제 RGB 림 색·윤곽을 관찰하고 품질 기준을 통과한 결손만 보완한다. |
...
`PlateMaskService`가 정리한 `plate_mask`에서 `plate_alpha`를 만들고, 음식 SAM 알파와 `maximum` 연산으로 합친다. 음식 지지 구조가 안전 조건을 통과하면 `food_support_mask`도 음식 알파와 보호 마스크에 합친다. 안전 제거와 분리 컴포넌트 제거 후에도 `plate_alpha`를 마지막에 다시 합쳐 접시 내부가 배경처럼 뚫리지 않게 한다.
...
- 음식 지지 구조는 기하학적 선만으로 채택하지 않는다(`allow_geometry_only: false`). 접시 경계 통과, 음식 연결과 SAM 계열 후보 증거가 필요하다.
- 적응형 림 관찰 신뢰도가 `minimum_confidence: 0.53`보다 낮거나 음식·접시 품질 기준을 통과하지 못하면 합성 림 브리지와 접시 알파 확장을 적용하지 않는다.
- 음식 지지 구조를 후처리 뒤 최상단 RGB 레이어에 다시 합성하는 실험은 롤백되었다. 현재 실행 단계에는 `step_5e_food_support_layer`가 없다.
...
보고서의 `debug_artifacts`에는 SAM/HQ-SAM 마스크, `plate_mask`, `plate_alpha`, `food_active_mask`, `food_support_mask`, `safe_removal_mask`, 분리 전경 제거 마스크, 컨테이너 블러 마스크, 접시 림 복원·알파 확장 마스크, 의미 비교 이미지와 최종 또는 거부 합성 이미지 경로가 기록된다.

최신 마스크·림 단계는 다음 보고서 키로 확인한다.

| 보고서 단계 | 확인 내용 |
| --- | --- |
| `step_2c_mask_quality` | 음식과 접시 마스크의 통과 여부, 신뢰도와 실패 이유 |
| `step_2d_food_support_recovery` | Hough 후보 수, 채택 컴포넌트 수, 복구 픽셀과 적용 여부 |
| `step_5d_plate_edge_repair` | 용기 형태, 적응형 림 관찰 신뢰도, 실제 림 보존·브리지·알파 확장 픽셀 |

## 일반화 회귀 검증

`scripts/build_container_generalization_regression.py`는 음식 이미지 묶음에서 접시·그릇·트레이·컵과 외관 조건을 나누어 고정 회귀 세트를 만든다. `scripts/evaluate_container_generalization_regression.py`는 각 실행 보고서의 음식·접시 품질, 용기 형태, 적응형 림 사용 여부와 브리지 픽셀을 집계한다.

이 스크립트가 존재한다는 사실과 실제 50장 E2E 평가가 완료됐다는 주장은 다르다. 집계 보고서가 생성되기 전에는 모든 음식·용기에 일반화됐다고 판단하지 않는다.
