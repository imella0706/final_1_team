...
→ 용기 형태 판정·음식/접시 품질 검사·음식 연결 지지 구조 보호
...
생성기는 음식·접시·컵이 없는 배경 후보를 기본 3장 생성한다. 중앙 여백, 색온도, YOLO 음식 미검출 여부로 후보를 고른다. preserve 모드의 원본 음식·접시는 캔버스 너비 기준 탑뷰 `0.40`, 45도 `0.48`을 기본 목표로 재배치하며 허용 범위는 0.30~0.70이다. 탑뷰는 중앙·약한 원형 그림자, 45도는 하단 중앙·일반 접지 그림자를 사용한다.

전경 보존 문제는 파이프라인 보고서의 `step_2c_mask_quality`, `step_2d_food_support_recovery`, `step_5d_plate_edge_repair`에서 확인한다. 림 관찰 신뢰도가 기준보다 낮으면 잘못된 선을 추가하지 않도록 브리지와 알파 확장을 생략하며, 이 경우 네이버 응답이 성공하더라도 해당 림 결손이 복구됐다고 볼 수는 없다.
