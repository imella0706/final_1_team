...
=======
19. publish_body는 네이버 블로그 편집기에 그대로 붙여넣을 수 있는 완성 원고로 작성하세요. UI 라벨, JSON 키 이름, 개발용 설명을 넣지 마세요.
20. publish_body에는 제목, 대표 사진 삽입 위치, 짧은 도입부, 사진별 본문 문단, 방문/주문 안내, 마무리, 해시태그가 자연스럽게 이어지도록 빈 줄을 포함하세요.
21. 문단은 모바일에서 읽기 좋게 1~3문장 단위로 짧게 나누고, 사진 위치는 "[사진 삽입: 사진 2 - 대표 메뉴]"처럼 복붙 후 사용자가 바로 위치를 알 수 있게 표시하세요.
22. blog_sections의 body도 복붙 원고에 들어갈 문단처럼 자연스러운 문장으로 쓰고, 너무 작은 UI 안내문처럼 보이는 짧은 조각 문장만 나열하지 마세요.
23. blog_sections의 body에는 작성 방향이 아니라 실제 독자에게 보여줄 본문을 넣으세요. 예: "오후의 조각의 수제 딸기 티라미수는 부드러운 크림과 딸기의 산뜻함이 어울리는 디저트입니다."
24. "정보형 문체로 소개합니다", "강조 포인트: 맛, 분위기", "type=디저트", "대표사진 추천" 같은 메타 설명이나 분석 라벨은 blog_sections.body와 publish_body에 절대 넣지 마세요.
25. 각 blog_sections.body는 최소 2문장 이상으로 작성하고, 상품명/상호명/특징/방문 맥락이 실제 문장 안에 들어가야 합니다.
26. 사진 관련 정보는 photo 필드에는 짧게 쓰되, body에는 그 사진을 보고 독자가 이해할 수 있는 메뉴 설명과 방문 이유를 자연스러운 문단으로 작성하세요.
>>>>>>> 2cc874ebad3ee688cf9812f5ede5f4213a4992f2
--------------------------------------------------
출력 JSON 스키마
--------------------------------------------------
{{
